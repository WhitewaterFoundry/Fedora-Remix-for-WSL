...
sudo curl -f https://raw.githubusercontent.com/WhitewaterFoundry/Fedora-Remix-for-WSL/master/linux_files/os-release -o /etc/os-release.tmp
(
  source /etc/os-release.tmp && sudo sed -i "s/\(FEDORA_REMIX_VERSION=\)\(.*\)/\1${FEDORA_REMIX_VERSION}/" /etc/os-release
)
sudo rm -f /etc/os-release.tmp
